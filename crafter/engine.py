...
class LocalView:
    ...
    def __call__(self, player, unit):
        ...
        for obj in self._world.objects: # TODO wrong refactoring?
...
class SemanticView:
    ...
    def __init__(self, world, obj_types):
        ...
        self._mat_names = world._mat_names.copy()
        self._obj_ids = {
    ...
    def semantic_to_onehot(self, semantic):
      from mv.utils import objects
      w, h = semantic.shape
      result = np.zeros(shape=(len(objects), h, w), dtype=np.uint8)
      result[objects['none-material']['index'], :, :] = 1
      result[objects['none-object']['index'], :, :] = 1
      for i in range(h):
        for j in range(w):
          value = semantic[i, j]
          if value < len(self._mat_names) + 1:
            result[objects[self._mat_names[value]]['index'], i, j] = 1
            result[objects['none-object']['index'], i, j] = 1
          else:
            result[objects['none-material']['index'], i, j] = 1
            obj_index = list(self._obj_ids.values()).index(value)
            objs = list(self._obj_ids.keys())
            obj = objs[obj_index].texture
            result[objects[obj], i, j] = 1
      return result
